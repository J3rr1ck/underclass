...
impl ShellType {
    pub fn parse(s: &str) -> Option<Self> {
        match s.to_lowercase().trim() {
            ...
            "sh" | "posix" => Some(ShellType::Sh),
            "adb" | "android" | "adbsh" => Some(ShellType::Adb),
            _ => None,
        }
    }
}
...
pub fn detect_installed_shells() -> Vec<ShellType> {
    ...
    if which_bin("sh").is_some() || home.join(".profile").exists() {
        detected.push(ShellType::Sh);
    }
    if which_bin("adb").is_some() {
        detected.push(ShellType::Adb);
    }
...
pub fn which_bin(name: &str) -> Option<String> {
    let candidates = match name {
        ...
        "fish" => vec!["/usr/bin/fish", "/bin/fish", "/usr/local/bin/fish", "/opt/homebrew/bin/fish"],
...
"#;
    ...
    let sh_plugin_path = dir.join("danger.plugin.sh");
    let sh_plugin_content = r#"# danger shell integration for POSIX sh
alias danger='under'
alias adbsh='danger shell adb'
"#;
    write(&sh_plugin_path, sh_plugin_content).map_err(|e| e.to_string())?;
...
pub fn stage_adb_shim() -> Result<PathBuf, String> {
    let bin_dir = shell_dir().join("bin");
    create_dir_all(&bin_dir).map_err(|e| e.to_string())?;
    let shim = bin_dir.join("danger-adb");

    let current_exe = std::env::current_exe().unwrap_or_else(|_| PathBuf::from("danger"));
    let shim_content = format!(
        "#!/bin/sh\n# Android ADB system shell wrapper for danger\nexec {} shell adb \"$@\"\n",
        current_exe.display()
    );
    write(&shim, shim_content).map_err(|e| e.to_string())?;
    set_permissions(&shim, Permissions::from_mode(0o755)).map_err(|e| e.to_string())?;
...
pub fn install_shell_plugin(shell: ShellType, login_shell: bool) -> Result<(), String> {
    ...
    match shell {
        ...
        ShellType::Sh => {
            ...
            let plugin_path = shell_dir().join("danger.plugin.sh");
            let block = format!(
                "{MARKER_START}\n[ -r \"{}\" ] && . \"{}\"\n{MARKER_END}",
                plugin_path.display(), plugin_path.display()
            );
            let existing = read_to_string(&profile).unwrap_or_default();
            if existing.contains(MARKER_START) {
                let re = regex::Regex::new(&format!("{}[\\s\\S]*?{}", regex::escape(MARKER_START), regex::escape(MARKER_END))).unwrap();
                let updated = re.replace(&existing, block.as_str()).to_string();
                write(&profile, updated).map_err(|e| e.to_string())?;
                println!("{}", format!("danger: updated POSIX sh integration in {}", profile.display()).yellow());
            } else {
                let sep = if existing.is_empty() || existing.ends_with('\n') { "" } else { "\n" };
                let new_content = format!("{existing}{sep}\n{block}\n");
                write(&profile, new_content).map_err(|e| e.to_string())?;
                println!("{}", format!("danger: added POSIX sh integration to {}", profile.display()).green());
            }
        }

        ShellType::Adb => {
            let bin_dir = stage_adb_shim()?;
            println!("{}", format!("danger: generated Android ADB shell helper at {}", bin_dir.join("danger-adb").display()).green());
            println!("  Usage: Run {} or `danger shell adb` to launch Android System Shell.", "danger-adb".cyan().bold());
        }
    }
...
pub fn uninstall_shell_plugin(shell: ShellType) -> Result<(), String> {
    ...
    match shell {
        ...
        ShellType::Sh => {
            let profile = home.join(".profile");
            if profile.exists() {
                let content = read_to_string(&profile).unwrap_or_default();
                let re = regex::Regex::new(&format!("\\n?{}[\\s\\S]*?{}\\n?", regex::escape(MARKER_START), regex::escape(MARKER_END))).unwrap();
                let cleaned = re.replace_all(&content, "\n").to_string();
                write(&profile, cleaned).map_err(|e| e.to_string())?;
                println!("{}", format!("danger: removed POSIX sh integration from {}", profile.display()).green());
            }
        }
        ShellType::Adb => {
            let shim = shell_dir().join("bin").join("danger-adb");
            if shim.exists() {
                let _ = std::fs::remove_file(&shim);
                println!("{}", format!("danger: removed Android ADB shell helper from {}", shim.display()).green());
            }
        }
    }
    Ok(())
...
pub fn start_subshell_for(shell: ShellType) -> i32 {
    ...
    if shell == ShellType::Adb {
        let adb_bin = which_bin("adb").unwrap_or_else(|| "adb".to_string());
        println!("{}", "danger shell — interactive Android system shell (adb shell) with danger integration loaded.".cyan().bold());

        // Probe connected ADB devices
        let devices_output = Command::new(&adb_bin).arg("devices").output();
        if let Ok(out) = devices_output {
            let dev_str = String::from_utf8_lossy(&out.stdout);
            let active_devs: Vec<&str> = dev_str.lines().skip(1).filter(|l| l.contains("device")).collect();
            if active_devs.is_empty() {
                println!("  {} No active Android ADB devices or emulators attached.", "!".yellow());
                println!("    Start an emulator or connect a device with USB debugging enabled.");
            } else {
                for dev in active_devs {
                    println!("  {} Attached ADB Device: {}", "✓".green(), dev.bold());
                }
            }
        }

        let status = Command::new(&adb_bin).arg("shell").status();
        return match status {
            Ok(s) => s.code().unwrap_or(0),
            Err(e) => {
                eprintln!("Failed to launch ADB shell {adb_bin}: {e}");
                1
            }
        };
    }
